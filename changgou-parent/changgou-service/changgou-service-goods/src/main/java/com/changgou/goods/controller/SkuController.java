...
public class SkuController {
    ...
    public Result rollBackCount(@RequestBody OrderItem orderItem){
        skuService.rollBackCount(orderItem);
    ...
    public Result decrCount(@RequestBody OrderItem orderItem){
        //库存递减
        skuService.decrCount(orderItem);
    ...
    public Result<List<Sku>> findByStatus(@PathVariable(name = "status")String status){
        List<Sku> byStatus = skuService.findByStatus(status);
        return new Result<>(true ,StatusCode.OK , "查询上架商品完成" , byStatus);
    }
