...
public class SeckillOrderController {
    ...
    public Result closeOrder(String username){
        seckillOrderService.closeOrder(username);
        return new Result(true,StatusCode.OK,"关闭回滚订单成功");
    }

    @GetMapping("/check")
    public Result query(String username) {
//        Map<String, String> map = tokenDecode.dcodeToken();
//        String username1 = map.get("username");
        SeckillStatus query = seckillOrderService.query(username);
        if(query != null){
            return new Result(true , query.getStatus(), "抢单成功，查询抢单状态" , query);
        }
        return new Result(false , StatusCode.NOTFOUNDERROR , "抢单失败，没有抢购信息");
    }

    @GetMapping("/add/redisOrder")
    public Result addRedisOrder(String id, String key, String username) {
//        Map<String, String> map = tokenDecode.dcodeToken();
//        String username1 = map.get("username");

        Boolean flag = seckillOrderService.addRedisOrder(id, key, username);
        if(flag){
            return new Result(true , StatusCode.OK ,"购买成功");
        }
        return new Result(false , StatusCode.ERROR , "服务器繁忙请稍后再试");
    }
