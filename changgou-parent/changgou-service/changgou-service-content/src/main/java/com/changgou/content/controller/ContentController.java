...
public class ContentController {
    ...
    public Result<List<Content>> findByCateId(@PathVariable(value = "id")Long id){
        List<Content> byCateId = contentService.findByCateId(id);
        return new Result<List<Content>>(true ,StatusCode.OK,"通过分类id查询广告内容成功",byCateId);
    }
    /***
     * Content分页条件搜索实现
     * @param content
    ...
    public Result<List<Content>> findByCategory(@PathVariable(name="id") Long id){
       List<Content>  contents = contentService.findByCategory(id);
        System.out.println("传入数据为："+id);
        for (Content content:contents) {
            System.out.println("查询到的结果为"+content);
        }
        if(contents == null){
            System.out.println("查询不到数据");
        }
       return new Result<List<Content>>(true,StatusCode.OK,"成功？！！",contents);
    }
}
